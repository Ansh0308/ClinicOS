...
export default function DoctorSignup() {
  ...
  const onStep1 = async (data) => {
    ...
    try {
      ...
      setFormData(data); setStep(2); startResendTimer()
  ...
  const onSubmitCode = async () => {
    ...
    if (!/^CLIN-[A-Z0-9]{4}$/.test(clinicCode.toUpperCase())) {
      setCodeError('Format must be CLIN-XXXX (e.g. CLIN-4X9Z)')
      return
    }
  ...
  return (
    <SignupLayout>
      ...
      {step === 1 && (
        <>
          <h2 className="font-display font-bold text-2xl text-text-primary mb-1">Doctor registration</h2>
          ...
          <form onSubmit={handleSubmit(onStep1)} className="space-y-4">
            <Field label="Full Name" error={errors.name?.message}>
              <input {...register('name')} placeholder="Dr. Ananya Roy" className={inputCls(errors.name)} />
            </Field>
            ...
            <Field label="Qualification" error={errors.qualification?.message}>
              <input {...register('qualification')} placeholder="MBBS, MD — Cardiology" className={inputCls(errors.qualification)} />
            </Field>
      ...
      {step === 3 && (
        <>
          ...
          <p className="font-body text-sm text-text-muted mb-6">
            Ask your clinic admin for the <strong>CLIN-XXXX</strong> code
          </p>
          <div className="mb-4">
